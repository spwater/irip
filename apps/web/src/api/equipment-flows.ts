...
export type Equipment = {
  ...
  description: string | null; visible_departments: string[];
...
export async function apiPublishComponent(body: { manifest_yaml: string; experimental_object_code?: string | null; equipment_id?: string | null; department_id?: string | null; visible_departments?: string[] | null; }): Promise<ComponentSummary> { const res = await http.post<ComponentSummary>('/components/', body); return res.data; }
...
export type FlowRunSummary = { id: string; flow_version_id: string; status: string; job_id: string | null; output_digest: string | null; output_summary: Record<string, unknown> | null; error_message?: string | null; started_at: string | null; completed_at: string | null; created_at: string; persisted_as_fact?: boolean; operator?: string | null; source_filename?: string | null; fact_id?: string | null; };
...
export async function apiCreateFlow(body: { display_name: string; department_id?: string | null; project_id?: string | null; operator: string; experimental_object_code?: string | null; nodes?: FlowNodeSchema[]; edges?: FlowEdgeSchema[]; }): Promise<FlowSummary> { const res = await http.post<FlowSummary>('/flows/', body); return res.data; }
...
export async function apiListFlows(params?: { status?: string; project_id?: string; }): Promise<CursorPage<FlowSummary>> { const res = await http.get<{ items: FlowSummary[] }>('/flows/', { params }); return { items: res.data.items, next_cursor: null, has_more: false }; }
...
export async function apiUpdateFlow(flowId: string, displayName: string, departmentId?: string | null, operator?: string | null, projectId?: string | null, experimentalObjectCode?: string | null): Promise<FlowSummary> { const res = await http.patch<FlowSummary>(`/flows/${flowId}`, { display_name: displayName, department_id: departmentId ?? null, project_id: projectId ?? null, operator: operator ?? null, experimental_object_code: experimentalObjectCode ?? null }); return { ...res.data, latest_version: res.data.latest_version ?? null }; }
