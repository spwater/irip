...
class TestParameterStaleness:
    ...
    async def test_staleness_detection(
        ...
        """事实修订后参数变为 review_required。

        流程：
        1. 创建推导链（含事实）；
        2. 创建参数、候选、审批 → 创建参数版本（含 staleness 跟踪）；
        3. 检查 staleness → current；
        4. 修订一个事实（创建新修订）；
        5. 再次检查 staleness → review_required。
        """
        org_id = param_setup["organization_id"]
        actor_id = param_setup["actor_id"]
        reviewer_id = new_id()
        ...
        first_fact_id = fact_refs[0].fact_id
        await fact_service.revise(
            fact_id=first_fact_id,
            reason="数据修正",
        )

        # 5. 再次检查 → review_required
        state = await param_service.check_staleness(version_ref.version_id)
