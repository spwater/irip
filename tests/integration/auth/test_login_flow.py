...
def test_refresh_token_is_single_use(api_client, seeded_user) -> None:
    ...
    rotated = api_client.post("/api/v1/auth/refresh", cookies={"irip_refresh": first_cookie})
    replay = api_client.post("/api/v1/auth/refresh", cookies={"irip_refresh": first_cookie})
...
def test_refresh_rotated_token_works(api_client, seeded_user) -> None:
    ...
    # 新令牌应可再次旋转
    second_rotate = api_client.post("/api/v1/auth/refresh", cookies={"irip_refresh": new_cookie})
...
def test_logout_invalidates_session(api_client, seeded_user) -> None:
    ...
    logout = api_client.post("/api/v1/auth/logout", cookies={"irip_refresh": cookie})
    assert logout.status_code == 200
    assert logout.json()["ok"] is True

    # 登出后刷新应失败
    refresh = api_client.post("/api/v1/auth/refresh", cookies={"irip_refresh": cookie})
...
def test_me_with_invalid_token(api_client) -> None:
    ...
    me = api_client.get("/api/v1/me", headers={"Authorization": "Bearer invalid.jwt.token"})
    assert me.status_code == 401
    assert me.json()["error"]["code"] == "invalid_credentials"
