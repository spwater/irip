...
class TestTrustedEntities:
    ...
    def test_table_names_have_research_prefix(self):
        ...
        for cls in [ResearchAnalysisPlanVersion, ResearchAnalysisRun, ResearchAnalysisStep,
                    ResearchRunArtifact, ResearchAiConversation, ResearchMemoryDocument]:
...
class TestMigration0075:
    ...
    def test_revision_numbers(self):
        ...
        # 直接读文件内容验证
        import pathlib
        path = pathlib.Path(__file__).parent.parent / "migrations" / "versions" / "0075_research_trusted_execution.py"
        text = path.read_text()
...
class TestContextRouter:
    ...
    def test_budget_hard_limit_500k(self):
        """500K 硬上限。"""
        from packages.research.context_router import ContextRouter, DATA_BUDGET_HARD_LIMIT
    ...
    def test_mode_full_compute(self):
        ...
        from packages.research.context_router import ContextRouter
        from packages.research.models_trusted import PlanStep, DataProfile, AnalysisMode
        router = ContextRouter()
        step = PlanStep(step_key="s1", question="计算均值", requires_full=True, per_record_semantic=False, cross_record_reasoning=False)
    ...
    def test_mode_chunked_when_over_budget(self):
        ...
        from packages.research.models_trusted import PlanStep, DataProfile, AnalysisMode
        router = ContextRouter()
        step = PlanStep(step_key="s1", question="逐条分析", requires_full=True, per_record_semantic=True, allows_sampling=False)
    ...
    def test_mode_direct_when_within_budget(self):
        ...
        from packages.research.models_trusted import PlanStep, DataProfile, AnalysisMode
        router = ContextRouter()
        step = PlanStep(step_key="s1", question="逐条分析", requires_full=True, per_record_semantic=True)
        profile = DataProfile(snapshot_id=uuid4(), total_tokens_estimate=100)
        mode, reason = router.analyze_step(step, profile)
    ...
    def test_no_silent_sampling(self):
        ...
        from packages.research.models_trusted import PlanStep, DataProfile, AnalysisMode
        router = ContextRouter()
        step = PlanStep(step_key="s1", question="跨记录推理", requires_full=True, cross_record_reasoning=True, allows_sampling=False)
        profile = DataProfile(snapshot_id=uuid4(), total_tokens_estimate=999999)
        mode, reason = router.analyze_step(step, profile)
...
class TestResearchScheduler:
    ...
    def test_acquire_slot_fails_at_21(self):
        """21 个用户 → 排队。"""
        from packages.research.scheduler import ResearchScheduler, MAX_CONCURRENT_USERS
        from unittest.mock import MagicMock
        redis = MagicMock()
    ...
    def test_release_slot_frees_resources(self):
        ...
        asyncio.get_event_loop().run_until_complete(
            scheduler.release_slot("user1", "run1")
        )
...
class TestModuleIsolation:
    ...
    def test_core_tables_no_fk_to_research_trusted(self):
        ...
        from packages.provenance.entities import EvidenceSet, DerivationRun
...
class TestAPIRoutes:
    ...
    def test_router_has_15_endpoints(self):
        ...
        from apps.api.routers.research_run import research_run_router
        routes = [
            r for r in research_run_router.routes
    ...
    def test_composition_registers_research_run(self):
        ...
        import pathlib
        comp_path = pathlib.Path(__file__).parent.parent / "apps" / "api" / "composition" / "__init__.py"
...
class TestScopeBoundary:
    ...
    def test_snapshot_changed_is_violation(self):
        """快照变更检测为越界。"""
        from packages.research.models_trusted import ScopeBoundary, ScopeCheckResult
        orch = self._make_orchestrator()
        scope = ScopeBoundary(snapshot_id=uuid4(), question_version=1)
