...
class TestConditionAST:
    ...
    def test_condition_ast_evaluates_whitelisted_fields(
    ...
    def test_condition_rejects_non_whitelisted_field(
        self, engine: ConditionEngine
    ) -> None:
        """非白名单字段被拒绝。"""
    ...
    def test_condition_nested_and_or(
        self, engine: ConditionEngine
    ) -> None:
        """嵌套 AND/OR 组合正确求值。
    ...
    def test_condition_lt_lte_gt_gte(
        self, engine: ConditionEngine
    ) -> None:
    ...
    def test_condition_in_not_matching(
        self, engine: ConditionEngine
    ) -> None:
        """in 操作符不匹配时返回 False。"""
        condition = {
    ...
    def test_condition_field_not_in_context(
        self, engine: ConditionEngine
    ) -> None:
        """上下文中缺少字段时返回 False。"""
