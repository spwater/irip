...
- 确认数据后创建不可变快照，并自动推荐 1–4 个起始问题；默认目标为 2 个，不为凑数生成低价值问题。
- 用户可以修改推荐问题，也可以自行提出问题。
- 后续推荐不自动出现。用户点击“帮我想下一步”时，系统再推荐 1–4 个问题；仍以 2 个为默认目标。
...
用户确认数据后，Snapshot Service 创建不可变快照。首次有效快照创建成功后，Recommendation Service 自动生成一批起始问题。结构化输出允许 1–4 个不重复问题，提示词默认请求 2 个；只有存在相互独立且有明确数据依据的额外方向时才增加到 3–4 个，只有一个方向值得研究时允许只返回 1 个。系统不得为了满足数量而填充低价值问题。推荐失败不会阻止用户手动提问。
...
执行过程复用现有 Research Plan/Run、统计工具、Artifact 和进度能力。完成后，时间线展示完整结果，并创建独立的候选结论提取任务。

候选提取由 Celery Worker 执行，不由前端请求触发，也不是分析步骤内部的附带动作。Research Run 成功事务提交后，服务端持久化 `CandidateExtractionJob`，再投递 Celery 任务；用户关闭页面后任务仍会继续。任务状态和结果写入数据库，前端优先通过现有 Run SSE 通道接收 `candidate_extraction.*` 事件，断线、刷新或错过事件时通过 Turn 详情轮询恢复。投递失败或 Worker 失联由定时扫尾任务重新投递或标记为明确失败。
...
时间线按发生时间展示研究轮次，不显示树形层级。服务端使用不透明游标分页，按 `(turn_number, id)` 倒序查询，默认每页 20 条、最大 50 条；前端首屏只取第一页，通过“加载更多”或等价的增量加载继续获取，不能全量拉取整个 Workspace。刷新后从第一页恢复，已展开轮次按 ID 保持本地界面状态。
...
| `RecommendationBatch` | 一次推荐结果 | 包含 1–4 个不重复问题，默认目标 2 个；未采用内容不进入正式时间线 |
...
| `CandidateExtractionJob` | 整轮分析完成后的异步候选提取 | 关联 Turn 和 Run；状态持久化，可重试且不依赖页面在线 |
...
| Recommendation Service | 首次自动推荐和后续按需推荐 1–4 个问题，默认目标 2 个并执行质量去重 |
| Turn Service | 轮次创建、输入固定、状态流转和并发控制 |
| Plan Service | 使用固定问题、快照和结论生成可编辑方案 |
| Run Service | 复用 Research Run、统计工具、进度、取消和 Artifact |
| Candidate Extraction Service | 在整轮 Run 完成后创建和执行 Celery 提取任务，保存状态、候选与重试信息 |
| Conclusion Service | 候选保存、人工新增和修订历史 |
| Synthesis Service | 综合所选结论并输出可空但类型稳定的结构 |
| Timeline Query Service | 使用游标分页组装时间线、当前状态、结论引用和访问限制 |
...
推荐问题、分析方案、候选结论和综合结果均采用严格结构化输出。推荐问题 schema 的 `minItems=1`、`maxItems=4`，并在服务端按规范化文本去重；数量或结构越界视为解析失败，不截断后静默保存。解析失败可以在受限次数内重试；仍失败时进入明确失败状态，不展示残缺对象。系统不保存或展示模型思维链。
...
- `summary`：综合判断，必须提供非空文本；
- `agreements`：共识；
- `conflicts`：冲突；
- `limitations`：限制；
- `new_hypotheses`：由现有结论推导出的待验证假设或新方向。

除 `summary` 外，每个分区都使用 `{status, items}` 结构。`status` 只能是 `present` 或 `not_applicable`；当本次确实不存在相关内容时使用 `not_applicable` 且 `items=[]`，不得为了填满模板生成“无明显冲突”等噪音。`present` 必须至少有一项。这样保留前端和 API 的稳定性，同时允许某次综合把新假设作为主要产出，而不是被固定文案弱化。
...
- 候选提取页面关闭：Celery 任务继续执行；重新打开页面后从持久化状态恢复，不依赖原 SSE 连接。
- 候选提取任务重复投递：以 `run_id` 唯一约束和任务状态机保证幂等，不重复创建候选。
...
- 单元测试：轮次状态、版本固定、结论修订、推荐 1–4 条边界与去重、综合可空分区、并发和幂等规则。
...
- 恢复测试：重复提交、候选提取投递失败、任务失联、Worker/Redis 中断、页面关闭和刷新。
- 前端测试：时间线游标分页、状态、结论勾选、快照过期提示、方案确认、候选提取 SSE/轮询恢复和并发禁用。
...
3. 第一次确认数据后得到 1–4 个可编辑 AI 推荐问题；正常情况下目标为 2 个，但不会为了凑数补低价值问题。
4. 后续推荐只在用户点击“帮我想下一步”后发生，同样遵循 1–4 个、默认目标 2 个的规则。
...
10. 分析完成后由独立 Celery 任务自动产生候选结论；关闭页面不影响任务，SSE 断线后可通过轮询恢复；只有用户保存的内容进入结论库。
11. 用户可以修改 AI 候选或新增无证据人工结论，并看到真实来源标识。
12. 新数据产生新快照，历史分析和结论继续绑定旧快照。
13. 选择多条结论可以完成结构化综合；除综合判断外，不适用分区可以为空且不会生成占位噪音。
14. 50 条以上轮次的 Workspace 使用 `(turn_number, id)` 倒序游标分页，首屏不会全量加载且翻页不重不漏。
15. 页面刷新后恢复快照、轮次、方案、任务状态、结果和结论。
16. 数据权限撤销后保留记录，但不返回敏感结果和 Artifact。
